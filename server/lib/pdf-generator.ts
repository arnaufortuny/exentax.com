...
export function generateOrderReceipt(orderData: {
  order: { id: number; requestCode: string; amount: number; currency: string; paymentMethod?: string | null; paidAt?: Date | null; };
  product: { name: string; description: string; features?: string[]; };
  user: { firstName?: string | null; lastName?: string | null; email: string; phone?: string | null; idType?: string | null; idNumber?: string | null; streetType?: string | null; address?: string | null; city?: string | null; province?: string | null; postalCode?: string | null; country?: string | null; };
  application?: { companyName?: string | null; designator?: string | null; state?: string | null; ein?: string | null; llcCreatedDate?: string | Date | null; registeredAgent?: string | null; };
  isMaintenance?: boolean;
}): Promise<Buffer> {
  const receiptData: ReceiptData = {
    orderNumber: orderData.order.id.toString(),
    requestCode: orderData.order.requestCode,
    date: (orderData.order.paidAt || new Date()).toISOString(),
    customer: {
      name: `${orderData.user.firstName || ''} ${orderData.user.lastName || ''}`.trim() || 'Cliente',
      email: orderData.user.email,
      phone: orderData.user.phone || undefined,
    ...
    service: {
      ...
      features: orderData.product.features,
    },
    llcDetails: orderData.application ? {
      companyName: orderData.application.companyName || undefined,
      designator: orderData.application.designator || undefined,
      state: orderData.application.state || undefined,
      ein: orderData.application.ein || undefined,
      creationDate: orderData.application.llcCreatedDate ? 
        (typeof orderData.application.llcCreatedDate === 'string' ? orderData.application.llcCreatedDate : orderData.application.llcCreatedDate.toISOString()) : undefined,
      registeredAgent: orderData.application.registeredAgent || undefined,
    } : undefined,
    amount: orderData.order.amount,
    currency: orderData.order.currency,
    paymentMethod: orderData.order.paymentMethod || undefined,
    isMaintenance: orderData.isMaintenance,
  };
